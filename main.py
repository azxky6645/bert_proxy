...
def define_args():
    ...
    p.add_argument('--data', type=str, default='Emory')
    p.add_argument('--model_dir', type=str, default='D:/ICAAI2', help="save model path")
...
def main(config):
    if config.wan >= 1:
        wandb.init(project="bert_proxy_third", entity="azxky6645", name=config.name, config=config.__dict__)
        wandb.config = {
    ...
    trainer.model.load_state_dict(torch.load(os.path.join(config.model_dir, config.data, config.name +'.pt'))['model'])
